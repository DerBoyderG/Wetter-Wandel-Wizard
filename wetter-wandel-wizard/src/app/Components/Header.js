...
const Header = ({ currentTime }) => {
  const [activeTab, setActiveTab] = useState('Heute'); // Hier sollte der aktive Tab gespeichert werden
...
<header className="bg-bg text-white p-4 flex justify-between items-center">
  ...
  <nav className="mt-4 flex">
    ...
    <a
      className={`text-white px-4 py-2 rounded transition duration-300 ease-in-out ${
        activeTab === 'Nächsten Tage' ? 'border-b-2 border-black font-bold' : 'hover:bg-blue-700'
      }`}
      onClick={() => setActiveTab('Nächsten Tage')}
    >
      Nächsten Tage
    </a>
  </nav>
